...
_CLASSNAMES = ["led_6core", "led_8core"]
...
class HhledDataset(torch.utils.data.Dataset):
    def __init__(
            self,
            source,
            classname,
    ...
    def sub_datasets(self, full_datasets, divide_num, divide_iter, random_seed=42):
        ...
        for k in id_dict.keys():
            ...
            sub_list = [
                devide_list[i][divide_iter]
                for i in range(len(devide_list))
                if len(devide_list[i]) > divide_iter
            ]
    ...
    def get_image_data(self):
        ...
        for classname in self.classnames_to_use:
            ...
            anomaly_types = self._list_dirs(classpath)
            ...
            for anomaly in anomaly_types:
                ...
                anomaly_files = self._list_files(anomaly_path)
                ...
                if self.split == DatasetSplit.TEST and anomaly != "good":
                    ...
                    anomaly_mask_files = self._list_files(anomaly_mask_path)
                    maskpaths_per_class[classname][anomaly] = {
                        x: os.path.join(anomaly_mask_path, x)
            ...
            for anomaly in sorted(imgpaths_per_class[classname].keys()):
                for image_path in imgpaths_per_class[classname][anomaly]:
                    ...
                    if self.split == DatasetSplit.TEST and anomaly != "good":
                        ...
                        mask_path = maskpaths_per_class[classname][anomaly].get(image_name)
                        if mask_path is None:
                            raise FileNotFoundError(
                                "Missing mask for image: {}".format(image_path)
                            )
